...
public class Door extends Terrain {
	...
	public void use() {
		...
		if (direction.equalsIgnoreCase("south") || direction.equalsIgnoreCase("north")) {// horizontal
			Creature c = super.getGame().getMap().getTile(new Point(super.getX(), super.getY() + 1)).getCreature();
			Creature c2 = super.getGame().getMap().getTile(new Point(super.getX(), super.getY() - 1)).getCreature();
			if (c != null) {
		...
		if (direction.equalsIgnoreCase("east") || direction.equalsIgnoreCase("west")) {// vertical
			Creature c = super.getGame().getMap().getTile(new Point(super.getX() + 1, super.getY())).getCreature();
			Creature c2 = super.getGame().getMap().getTile(new Point(super.getX() - 1, super.getY())).getCreature();
			if (c != null) {
				ArrayList<Item> in = c.getInventory();
				for (Item i : in) {
					if (i.getTags() != null) {
